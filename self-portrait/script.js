document.addEventListener("DOMContentLoaded", () => {
  ...
  function formatSecond(s) {
    if (s < 1) {
      ...
      return `${Math.floor(s / 60 / 60)}:${numToTwoDigit(
        s % (60 * 60)
      )}:${numToTwoDigit(s % 60)}`;
    }
  }
  ...
  function updateOpacity() {
    ...
    if (currentTime <= 1.4) {
      document.documentElement.style.setProperty(
        "--my-opacity",
        `${(currentTime - 2) * (currentTime - 2) * 0.48 + 0.12}`
      );
      return;
    }
    ...
    document.documentElement.style.setProperty(
      "--my-opacity",
      `${(currentTime - 2.3) * (currentTime - 2.3) * 0.55 - 0.15}`
    );
  }
  ...
  if (
    navigator.userAgent.match(/\b(?:iphone|ipad|ipod|macintosh)/i) &&
